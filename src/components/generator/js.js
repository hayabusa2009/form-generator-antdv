...
function buildAttributes(
  ...
  if (
    scheme["data-source"] &&
    (config.tag === "a-auto-complete" || config.tag === "a-transfer")
  ) {
  ...
  if (ruleTrigger[config.tag]) {
    if (config.required) {
      ...
      let message = isArray(config.defaultValue)
        ? `${i18nRender("base.select.least.one")}${config.label}`
        : scheme.placeholder;
      if (message === undefined)
        message = `${config.label}${i18nRender("base.not.null")}`;
      rules.push(
...
function buildReplaceFields(scheme, replaceFieldsList) {
  const str = `${scheme.__vModel__}ReplaceFields: ${JSON.stringify(
    scheme["replace-fields"]
  )},`;
...
function buildBeforeUpload(scheme) {
  ...
  if (config.fileSize) {
    ...
    if(!isRightSize){
      this.$message.error('${i18nRender("base.file.size.overflow")} ${
      config.fileSize
    }${config.sizeUnit}')
    }`;
  ...
  if (scheme.accept) {
    ...
    if(!isAccept){
      this.$message.error('${i18nRender("base.file.should.choose.type")}${
      scheme.accept
    }')
    }`;
...
export function makeUpSetup(conf, optionCodeStr) {
  const startIndex = exportDefault.length;
  const optionCode = strToJson(optionCodeStr.slice(startIndex));
  const data = optionCode.data();
  const { formModel } = conf;
  const formState = data[formModel];
  let setupCode = `
    const ${formModel} = reactive(
      ${JSON.stringify(formState)}
    );
  `;
  Reflect.deleteProperty(data, formModel);
  Object.keys(data).forEach((key) => {
    setupCode += `const ${key} = ${JSON.stringify(data[key])};`;
  });
  const { methods } = optionCode;
  const setupMethodsKeys = Object.keys(setupMethods);
  Object.keys(methods).forEach((key) => {
    if (setupMethodsKeys.includes(key)) {
      setupCode += setupMethods[key];
    } else {
      setupCode += `function ${methods[key]};`;
    }
  });
