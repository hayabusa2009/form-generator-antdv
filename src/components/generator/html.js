...
export function dialogWrapper(str) {
  return `<a-modal v-bind="$attrs" v-on="$listeners" @open="onOpen" @cancel="onClose" title="Form Titile">
    ...
    <div slot="footer">
      ...
      <a-button type="primary" @click="handelConfirm">${i18nRender(
        "base.ok"
      )}</a-button>
    </div>
  </a-modal`;
...
function buildFromBtns(scheme, type) {
  ...
  if (
    scheme.labelCol &&
    scheme.wrapperCol &&
    (!scheme.layout || scheme.layout === "horizontal")
  ) {
  ...
  if (scheme.formBtns && type === "file") {
    str = `<a-form-item ${wrapperCol}>
          <a-space>
          <a-button type="primary" @click="submitForm">${i18nRender(
            "base.submit"
          )}</a-button>
...
const layouts = {
  colFormItem(scheme) {
    ...
    if (
      config.wrapperCol &&
      (!config.layout || scheme.layout === "horizontal")
    ) {
    ...
    let str = `<a-form-item ${labelCol} ${wrapperCol} ${labelAlign} ${label}
      name="${scheme.__vModel__}" ${required}>
        ${tagDom}
      </a-form-item>`;
...
const tags = {
  "a-button": (el) => {
    const { tag, disabled, size } = attrBuilder(el);
  ...
  "a-input": (el) => {
    const { tag, disabled, vModel, allowClear, placeholder, width } =
      attrBuilder(el);
  ...
  "a-input-number": (el) => {
    const { tag, disabled, vModel, placeholder, width } = attrBuilder(el);
  ...
  "a-mentions": (el) => {
    ...
    const placement =
      el.placement !== "bottom" ? `placement='${el.placement}'` : "";
    const prefix = el.prefix !== "@" ? `prefix='${el.prefix}'` : "";
  ...
  "a-transfer": (el) => {
    ...
    const titles = el.titles
      ? `:titles="['${el.titles[0]}', '${el.titles[1]}']"`
      : "";
    const changeTargetKeys = `@change="change${titleCase(
      el.__vModel__
    )}TargetKeys"`;
  ...
  "a-radio-group": (el) => {
    const { tag, disabled, vModel, size } = attrBuilder(el, "checked");
    const buttonStyle =
      el.buttonStyle === "outline" ? "" : `button-style="${el.buttonStyle}"`;
  ...
  "a-switch": (el) => {
    const { tag, disabled, vModel, size } = attrBuilder(el);
    const checkedChildren = el["checked-children"]
      ? `checked-children="${el["checked-children"]}"`
      : "";
    const unCheckedChildren = el["un-checked-children"]
      ? `un-checked-children="${el["un-checked-children"]}"`
      : "";
  ...
  "a-cascader": (el) => {
    ...
    const showSearch = el.showSearch ? "show-search" : "";
    const expandTrigger =
      el["expand-trigger"] === "click"
        ? ""
        : `expand-trigger="${el["expand-trigger"]}"`;

    return `<${tag} ${vModel} ${options} ${props} ${width} ${changeOnSelect} ${placeholder}
      ${expandTrigger} ${showSearch} ${allowClear} ${disabled} />`;
  },
  "a-tree-select": (el) => {
    const { tag, disabled, vModel, allowClear, placeholder, width, size } =
      attrBuilder(el);
    const treeData = el["tree-data"]
      ? `:tree-data="${el.__vModel__}TreeData"`
      : "";
    const replaceFields = el["replace-fields"]
      ? `:replace-fields="${el.__vModel__}ReplaceFields"`
      : "";
    const showSearch = el.showSearch && !el.multiple ? "show-search" : "";
    const dropdownMatchSelectWidth = el["dropdown-matc-select-width"]
      ? "dropdown-matc-select-width"
      : "";
    // treeCheckStrictly会使得labelInValue强制为true
    const labelInValue =
      el["label-in-value"] && !el["tree-check-strictly"]
        ? "label-in-value"
        : "";
    const treeDefaultExpandAll = el["tree-default-expand-all"]
      ? "tree-default-expand-all"
      : "";
    const multiple = el.multiple && !el["tree-checkable"] ? "multiple" : "";
    const treeCheckable = el["tree-checkable"] ? "tree-checkable" : "";
    const treeCheckStrictly =
      el["tree-check-strictly"] && el["tree-checkable"]
        ? "tree-check-strictly"
        : "";
  ...
  "a-date-picker": (el) => {
    ...
    const showToday = el["show-today"]
      ? ""
      : `show-today="${el["show-today"]}"`;
  ...
  "a-range-picker": (el) => {
    const { tag, disabled, vModel, allowClear, width, size } = attrBuilder(el);
    const format = el.format ? `format="${el.format}"` : "";
    const valueFormat = el["value-format"]
      ? `value-format="${el["value-format"]}"`
      : "";
    ...
    const placeholder = el.placeholder
      ? `:placeholder="['${placeholders[0]}', '${placeholders[1]}']"`
      : "";
  ...
  "a-rate": (el) => {
    const { tag, disabled, allowClear, vModel } = attrBuilder(el);
...
function attrBuilder(el, modelValue = "value") {
  return {
    tag: el.__config__.tag,
    vModel: `v-model:${modelValue}="${confGlobal.formModel}.${el.__vModel__}"`,
...
function buildElInputChild(scheme) {
  ...
  if (slot && slot.addonBefore) {
    children.push(
      `<template slot="addonBefore"><a-icon type=${slot.addonBefore} /></template>`
    );
  }
  if (slot && slot.addonAfter) {
    children.push(
      `<template slot="addonAfter"><a-icon type=${slot.addonAfter} /></template>`
    );
  }
  return children.join("\n");
}
...
function buildElRadioGroupChild(scheme) {
  ...
  if (slot && slot.options && slot.options.length) {
    ...
    const buttonStyle =
      scheme.buttonStyle === "outline"
        ? ""
        : `button-style = ${scheme.buttonStyle}`;
...
function buildElUploadChild(scheme) {
  ...
  if (scheme["list-type"] === "picture-card") {
    list.push(
      `<div><a-icon type="plus" /><div class="ant-upload-text">${config.buttonText}</div></div>`
    );
  } else {
    list.push(
      `<a-button> <a-icon type="upload" /> ${config.buttonText}</a-button>`
    );
  }
  return list.join("\n");
}
